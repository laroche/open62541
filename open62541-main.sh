...
if test "X$1" = Xinstall ; then
  if grep -q Debian /etc/os-release ; then
    sudo apt-get install git cmake make libmbedtls-dev python3-sphinx texlive-base python3-pkgconfig check libpcap-dev clang-19 # libssl-dev cmake-qt-gui arm-none-eabi-gcc
  else
    sudo apt-get install git cmake make libmbedtls-dev python3-sphinx texlive-base python3-pkgconfig check libpcap-dev # libssl-dev cmake-qt-gui arm-none-eabi-gcc
  fi
  exit 0
fi

if test "X$1" = Xupload ; then
  rsync -av --delete --exclude=build . debian-13:data/open62541/
  exit 0
fi

# Checkout the source the first time:
if ! test -d open62541-main ; then
  #git clone https://github.com/open62541/open62541
  git clone https://github.com/open62541/open62541 open62541-main
  pushd open62541-main
    git config pull.rebase false
    patch -s -p1 < ../open62541-main.patch
...
if test "X$1" = Xpatch ; then
  pushd open62541-main
    git diff --ignore-submodules=dirty > ../open62541-main.patch2
    pushd deps/mdnsd
      git diff > ../../../open62541-mdnsd.patch2
    popd
  popd
  diff -u open62541-main.patch open62541-main.patch2
  diff -u open62541-mdnsd.patch open62541-mdnsd.patch2
  mv open62541-main.patch2 open62541-main.patch
